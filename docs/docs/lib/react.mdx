...
### Unit Testing and `fetch` Polyfills

If your React tests run in an environment that does not provide `fetch` (for example, older Node.js versions or non-browser test environments), GrowthBook's built-in feature loading (`init`, `loadFeatures`, `refreshFeatures`) requires a polyfill.

Set this once in your test setup file before creating any GrowthBook instances:
...
For Jest, add this to your `setupFilesAfterEnv` file. For Vitest, add it to `setupFiles`. If your environment already has `fetch` (for example, modern browsers or Node.js 18+), this is not required.
