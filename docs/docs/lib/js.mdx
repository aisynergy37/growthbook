...
If your tests run in an environment that does not provide `fetch` (for example, older Node.js versions or non-browser test environments), GrowthBook's built-in feature loading (`init`, `loadFeatures`, `refreshFeatures`) requires a polyfill.
